...
// 데이터 영속성 제어 (api.js 연동)
async function loadAll() {
  try {
    const r = await fsGet('roster');
    roster = r ? JSON.parse(r) : [];
  } catch (e) { showToast('불러오기 실패, 인터넷 연결을 확인해주세요'); }
  
  try {
    const rd = await fsGet('rounds');
    rounds = rd ? JSON.parse(rd) : [];
  } catch (e) { showToast('불러오기 실패, 인터넷 연결을 확인해주세요'); }
  
  try {
    const a = await fsGet('activeRoundId');
    activeRoundId = a ? JSON.parse(a) : null;
  } catch (e) { activeRoundId = null; }
}

async function saveRoster() {
  try { await fsSet('roster', JSON.stringify(roster)); }
  catch (e) { showToast('저장 실패, 다시 시도해주세요'); }
}

async function saveRounds() {
  try { await fsSet('rounds', JSON.stringify(rounds)); }
  catch (e) { showToast('저장 실패, 다시 시도해주세요'); }
}

async function saveActive() {
  try { await fsSet('activeRoundId', JSON.stringify(activeRoundId)); }
  catch (e) {}
}
...
(async function init() {
  ...
  // 실시간 동기화 구독
  subscribeCollection('roster', data => {
    roster = data || [];
    render();
  }, () => showToast('실시간 동기화 오류'));

  subscribeCollection('rounds', data => {
    rounds = data || [];
    render();
  }, () => showToast('실시간 동기화 오류'));

  subscribeCollection('activeRoundId', data => {
    activeRoundId = data || null;
    render();
  }, () => {});
})();